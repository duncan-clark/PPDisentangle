...
FIXED_BETA      <- 0.05        # temporal decay rate (1/beta ~ 20 day mean trigger time)
...
# --- Vanilla Hawkes fit ---
VANILLA_MAXIT   <- 5000        # max optim iterations for initial fits
VANILLA_STARTS  <- list(       # multi-start initial values
...
# --- SEM outer loop ---
SEM_N_LABELLINGS <- 20         # number of labelling proposals for outer importance weights
SEM_N_ITER       <- 2          # number of outer EM iterations

# --- SEM adaptive (inner) step ---
SEM_INNER_ITER          <- 50  # inner labelling iterations per adaptive step
SEM_INNER_N_PROPS       <- 20  # proposals per inner iteration
SEM_PARAM_UPDATE_CADENCE <- 10 # re-estimate params every N inner iterations
...
cat(sprintf("Starting radii study: %d radii, SEM inner=%d iters x %d props, outer=%d iters x %d labellings\n",
            length(RADII_KM), SEM_INNER_ITER, SEM_INNER_N_PROPS, SEM_N_ITER, SEM_N_LABELLINGS))
...
for (r_km in RADII_KM) {
  ...
  cat(sprintf(" done (K_trt=%.3f K_ctrl=%.3f)\n", vanilla_trtd[["K"]], vanilla_ctrl[["K"]]))
