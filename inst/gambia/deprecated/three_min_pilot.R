...
cat("Total points in study:", nrow(pp_data), "\n")
...
# =========================================================
# 2. Targeted Pilot Run (~3 minutes)
# =========================================================
cat("Starting targeted pilot run...\n")

# We'll test 3 change_factors and 3 n_props with 100 iterations each.
# This should take approximately 3-5 minutes.
grid <- expand.grid(
  change_factor = c(0.01, 0.05, 0.1),
  n_props = c(20, 50, 100)
)
...
for (i in 1:nrow(grid)) {
  ...
  cat(paste0("Testing cf=", cf, ", np=", np, " (", n_pilot_iter, " iterations)...\n"))
  ...
  cat(paste0("  Done in ", round(t_elapsed, 1), "s (", round(t_elapsed/n_pilot_iter, 2), "s/iter)\n"))
  
  pilot_results[[i]] <- data.frame(
...
final_res <- do.call(rbind, pilot_results)
print(final_res %>% group_by(change_factor, n_props) %>% 
        summarize(start_metric = first(metric), 
                  end_metric = last(metric), 
                  avg_flips_final = last(average_flips),
                  max_flips_final = last(max_metric_flips)))

cat("\nTargeted pilot run complete!\n")
