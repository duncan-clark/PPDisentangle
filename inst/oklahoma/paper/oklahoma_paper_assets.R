...
if (!is.null(input_arg) && nzchar(input_arg)) {
  ...
  input_candidates <- c(
    ...
    pp_output_path("oklahoma", "for_paper_biv_aon.rds", repo_root = repo_root),
...
# Explosive and failed refits are excluded by oklahoma_analysis.R. Keep every
...
target_mean_by_model <- c(
  "Naive Bivariate ETAS with KDE" = obs_E,
  "SEM Bivariate ETAS with KDE" = obs_F
)
attempted_by_model <- c(
  "Naive Bivariate ETAS with KDE" = boot_obj$fit_E$n_attempted %||%
    boot_obj$config$reps %||% nrow(boot_E),
  "SEM Bivariate ETAS with KDE" = boot_obj$fit_F$n_attempted %||%
    boot_obj$config$reps %||% nrow(boot_F)
)
explosive_by_model <- c(
  "Naive Bivariate ETAS with KDE" = boot_obj$fit_E$n_explosive %||% NA_integer_,
  "SEM Bivariate ETAS with KDE" = boot_obj$fit_F$n_explosive %||% NA_integer_
)
boot_df <- boot_df %>%
  group_by(.data$model) %>%
...
p_boot_hist <- ggplot(boot_df, aes(x = .data$ate_total_mean, fill = .data$model)) +
  ...
  scale_fill_manual(values = c(
    "Naive Bivariate ETAS with KDE" = "#e6550d",
...
# GR branching spectral radius: rho of [[eta_00,eta_01],[eta_10,eta_11]],
# eta_ij = A_ij * beta / (beta - alpha_ij). Explosive iff rho >= 1 (or Inf).
...
write_tex_ok_ef_params <- function(E_vec, F_vec, path, beta_gr = 2.3) {
  rho_E <- if (length(E_vec)) spectral_radius_etas(E_vec, beta_gr) else NA_real_
  rho_F <- if (length(F_vec)) spectral_radius_etas(F_vec, beta_gr) else NA_real_
  fmt_rho <- function(x) if (!is.finite(x)) "Inf" else fmt_sigfig(x, 3L)
  fmt_expl <- function(x) {
    if (!is.finite(x)) return("yes (Inf)")
    if (x >= 1) "yes" else "no"
  }
  lines <- c(
    "% Auto-generated by oklahoma_paper_assets.R — do not edit by hand",
    ...
    paste0(
      ...
      "plus Gutenberg--Richter branching spectral radius $\\rho$ ($\\beta=",
      signif(beta_gr, 3),
      "). Channel mean offspring $\\eta_{ij}=A_{ij}\\beta/(\\beta-\\alpha_{ij})$ ",
      "(Inf if $\\alpha_{ij}\\ge\\beta$). Explosive means $\\rho(\\eta)\\ge 1$.}"
    ),
    "\\begin{tabular}{@{}lcc@{}}",
    "\\toprule",
  ...
  lines <- c(
    lines,
    "\\midrule",
    sprintf("$\\rho$ (spectral radius) & %s & %s\\\\", fmt_rho(rho_E), fmt_rho(rho_F)),
    sprintf("explosive? ($\\rho\\ge 1$) & %s & %s\\\\", fmt_expl(rho_E), fmt_expl(rho_F)),
    "\\bottomrule", "\\end{tabular}", "\\end{table}"
  )
  writeLines(lines, path)
  message("Wrote ", path, sprintf(" (rho_E=%s rho_F=%s)", fmt_rho(rho_E), fmt_rho(rho_F)))
}
...
beta_gr_paper <- suppressWarnings(as.numeric(unlist(cfg_paper$BETA_GR, use.names = FALSE)))
beta_gr_paper <- beta_gr_paper[is.finite(beta_gr_paper) & beta_gr_paper > 0]
beta_gr_paper <- if (length(beta_gr_paper)) beta_gr_paper[[1]] else 2.3
write_tex_ok_ef_params(E_vec, F_vec, file.path(tex_dir, "tab_ok_ef_params.tex"), beta_gr = beta_gr_paper)
...
if (have_boot) {
  summ <- boot_df %>%
    ...
    mutate(
      n_attempted = as.integer(attempted_by_model[.data$model]),
      n_explosive = as.integer(explosive_by_model[.data$model])
    )

  lines_boot <- c(
    "% Auto-generated by oklahoma_paper_assets.R",
    "\\begin{table}",
    "\\caption{\\label{tab:ok_bootstrap_summary}Bootstrap summary after excluding failed or explosive refits; bias correction is applied to the stable remainder.}",
    "\\centering",
    "\\begin{tabular}[t]{@{}lrrrrrrr@{}}",
    "\\toprule",
    "Model & Retained/attempted & Explosive & Mean & SD & 2.5\\% & 50\\% & 97.5\\%\\\\",
    "\\midrule"
  )
  ...
  for (i in seq_len(nrow(summ))) {
    ...
    lines_boot <- c(
      ...
      sprintf(
        ...
        as.integer(summ$n_attempted[i]),
        if (is.na(summ$n_explosive[i])) "--" else as.character(summ$n_explosive[i]),
