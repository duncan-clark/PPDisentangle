...
PP_FITVAR_ATE_ONLY="${PP_FITVAR_ATE_ONLY:-0}"
...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    --fitvar-ate-only) PP_FITVAR_ATE_ONLY=1; PP_FIT_VARIABILITY_ONLY=1; shift ;;
...
if [ -n "$PP_MODE" ] && [ -z "${SLURM_JOB_ID:-}" ]; then
  ...
  case "$mode_norm" in
    ...
    fit-variability-only|fitvar-only|variability-only)
      ...
      if [ "$MEM_EXPLICIT" -ne 1 ]; then PP_MEM=160G; fi
      ;;
    fitvar-ate-only|fit-variability-ate-only)
      # Recompute fitvar ATEs from stored SEM params; do not refit.
      apply_paper_science_suite
      PP_FIT_VARIABILITY_ONLY=1
      PP_FITVAR_ATE_ONLY=1
      PP_FIT_VARIABILITY_D_ONLY=1
      PP_BOOT_REPS=0
      BOOT_REPS_EXPLICIT=1
      PP_RUN_SENSITIVITY=0
      RUN_SENS_EXPLICIT=1
      PP_RUN_PARTITION_SENSITIVITY=0
      PP_RUN_T_TRUNC_SENSITIVITY=0
    ...
    univ-kde-only|univ_kde_only|just-gh|just_gh|gh-only|gh_only)
      ...
      ;;
    *)
      echo "Unknown --mode '$PP_MODE' (expected: paper | paper-quick | paper-cd | init-smoke | test | quick | full | fit-variability | fit-variability-only | fitvar-ate-only | bootstrap-only | t-trunc-sens-only | smoke-sem-d | cd-primary | cd-only | univ-kde-only)"
      exit 1
      ;;
  esac
fi
...
if [ -z "${SLURM_JOB_ID:-}" ]; then
  ...
  export PP_RUN_SENSITIVITY PP_RUN_FIT_VARIABILITY PP_FIT_VARIABILITY_REPS PP_FIT_VARIABILITY_CORES PP_FIT_VARIABILITY_PATCH_FILE PP_FIT_VARIABILITY_ONLY PP_FIT_VARIABILITY_D_ONLY PP_FITVAR_ATE_ONLY PP_BOOTSTRAP_PATCH_FILE PP_BOOTSTRAP_ONLY PP_T_TRUNC_SENS_PATCH_FILE PP_T_TRUNC_SENS_ONLY PP_SMOKE_SEM_D_SEEDS PP_SMOKE_SEM_D_TRUNC PP_SKIP_FULL_REPORT PP_CD_ONLY PP_UNIV_KDE_ONLY PP_MEM PP_TIME PP_SEM_OPTIM_METHOD PP_SEM_SELECTION_TEMPERATURE
...
if [ "${PP_FITVAR_ATE_ONLY:-0}" = "1" ] || [ "${PP_FITVAR_ATE_ONLY:-0}" = "true" ]; then
  export OK_FITVAR_ATE_ONLY=true
  export OK_FIT_VARIABILITY_ONLY=true
else
  export OK_FITVAR_ATE_ONLY=false
fi
