...
DEFAULT_TEX_ALL_NOTHING_TABLE <- "tab_sim_all_nothing_true_control.tex"
...
# Nested table counterpart to fig:all_nothing (naive vs SEM only; no oracle).
# Two method blocks, each with one row per post-treatment time scaling.
build_all_nothing_table_tex_lines <- function(
    df,
    legend_info,
    target_multipliers = DEFAULT_TARGET_MULTIPLIERS,
    y_col = "all_nothing_true_control_per_post_time") {
  if (is.null(df) || nrow(df) < 1L) return(character(0))
  work <- df %>%
    mutate(method_chr = as.character(.data$method)) %>%
    filter(.data$method_chr %in% c("naive", "SEM")) %>%
    filter(vapply(
      .data$time_multiplier,
      function(z) any(abs(z - target_multipliers) < 1e-9),
      logical(1)
    ))
  if (nrow(work) < 1L) return(character(0))

  true_ate <- mean(work$true_ate_per_post_time, na.rm = TRUE)
  summ <- work %>%
    group_by(.data$method_chr, .data$time_multiplier) %>%
    summarize(
      n = dplyr::n(),
      mean_est = mean(.data[[y_col]], na.rm = TRUE),
      median_est = stats::median(.data[[y_col]], na.rm = TRUE),
      q25 = as.numeric(stats::quantile(.data[[y_col]], 0.25, na.rm = TRUE, names = FALSE)),
      q75 = as.numeric(stats::quantile(.data[[y_col]], 0.75, na.rm = TRUE, names = FALSE)),
      .groups = "drop"
    ) %>%
    mutate(
      method_label = ifelse(.data$method_chr == "naive", "Naive", "SEM"),
      mult_chr = formatC(.data$time_multiplier, format = "f", digits = 1)
    )

  if (!is.null(legend_info) && nrow(legend_info) > 0L &&
      all(c("time_multiplier", "avg_post_points") %in% names(legend_info))) {
    summ <- summ %>%
      left_join(
        legend_info[, c("time_multiplier", "avg_post_points"), drop = FALSE],
        by = "time_multiplier"
      )
  } else {
    summ$avg_post_points <- NA_real_
  }

  fmt_plain <- function(x, d = 3) {
    ifelse(is.finite(x), sprintf(paste0("%.", d, "f"), x), "NA")
  }
  fmt_math <- function(x, d = 3) {
    ifelse(is.finite(x), paste0("$", fmt_plain(x, d), "$"), "NA")
  }
  window_label <- function(m, pts) {
    if (is.finite(pts)) {
      sprintf(
        "$%s\\times$ ($\\approx$%.0f)",
        formatC(m, format = "f", digits = 1), pts
      )
    } else {
      sprintf("$%s\\times$", formatC(m, format = "f", digits = 1))
    }
  }

  method_levels <- c("Naive", "SEM")
  mults <- sort(unique(as.numeric(summ$time_multiplier)))
  body_blocks <- lapply(seq_along(method_levels), function(mi) {
    meth <- method_levels[[mi]]
    rows <- lapply(seq_along(mults), function(j) {
      m <- mults[[j]]
      hit <- summ %>%
        filter(
          .data$method_label == meth,
          abs(.data$time_multiplier - m) < 1e-9
        )
      if (nrow(hit) < 1L) {
        cells <- c("NA", "NA", "NA")
        win <- window_label(m, NA_real_)
      } else {
        cells <- c(
          fmt_math(hit$mean_est[[1]]),
          fmt_math(hit$median_est[[1]]),
          sprintf(
            "$[%s,\\,%s]$",
            fmt_plain(hit$q25[[1]]),
            fmt_plain(hit$q75[[1]])
        ...
        win <- window_label(m, hit$avg_post_points[[1]])
      }
      meth_cell <- if (j == 1L) {
        sprintf("\\multirow{%d}{*}{%s}", length(mults), meth)
      } else {
        ""
      }
      paste(c(meth_cell, win, cells), collapse = " & ")
    })
    out <- paste0(unlist(rows), " \\\\")
    if (mi < length(method_levels)) {
      out <- c(out, "\\midrule")
    }
    out
  })
  ...
  c(
    "% Auto-generated by inst/sim_study/plot_time_sweep_publication.R",
    "% Table counterpart to fig:all_nothing (naive and SEM only).",
    "% Requires \\usepackage{multirow} in the parent preamble.",
    "\\begin{table}[htbp]",
    "\\centering",
    paste0(
      "\\caption{Estimated all-treated versus all-control DAITE (per cell per unit ",
      "post-treatment time) with control parameters fixed at truth. For each method, ",
      "rows are post-treatment horizons (approximate mean post-treatment event counts ",
      "in parentheses). Columns report the Monte Carlo mean, median, and interquartile ",
      "range $[Q_1,Q_3]$. The true DAITE is $",
      fmt_plain(true_ate, 2),
      "$. Oracle labelling is omitted.}"
    ),
    "\\label{tab:all_nothing}",
    "\\begin{tabular}{llrrr}",
    "\\toprule",
    "Method & Post window & Mean & Median & IQR $[Q_1,Q_3]$ \\\\",
    "\\midrule",
    unlist(body_blocks),
    "\\bottomrule",
    "\\end{tabular}",
...
write_all_nothing_table_tex <- function(df, legend_info, target_multipliers, tex_file) {
  lines <- build_all_nothing_table_tex_lines(
    df, legend_info, target_multipliers = target_multipliers
  )
  if (length(lines) < 1L) return(invisible(NULL))
  writeLines(lines, con = tex_file, useBytes = TRUE)
  invisible(lines)
...
run_publication_plot <- function(
  ...
  tex_all_nothing_base <- if (use_custom) {
    paste0(output_prefix, "_all_nothing_table.tex")
  } else {
    DEFAULT_TEX_ALL_NOTHING_TABLE
  }
  ...
  tex_all_nothing_file <- file.path(tx_dir, tex_all_nothing_base)
  ...
  write_all_nothing_table_tex(
    df, legend_info,
    target_multipliers = target_multipliers,
    tex_file = tex_all_nothing_file
  )
  ...
  message("Wrote: ", tex_all_nothing_file)
  message("Legend info:")
  ...
  invisible(list(
    ...
    tex = tex_file,
    tex_all_nothing = tex_all_nothing_file
  ))
}
